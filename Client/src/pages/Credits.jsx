...
const Credits = () => {
  ...
  const fetchPlans = async () => {
    try {
      ...
      if (data.success) {
        setPlans(data.plans && data.plans.length > 0 ? data.plans : dummyPlans)
      } else {
        setPlans(dummyPlans)
      }
    } catch (err) {
      toast.error('Failed to load live credit plans, using offline fallback')
      setPlans(dummyPlans)
    } finally {
      setLoading(false)
    }
        ...
        {plans.map((plan) => (
          ...
          >
            <div className='flex-1'>
              ...
              <p className='text-2xl font-bold text-purple-600 dark:text-purple-300 mb-2'>
                ${plan.price}
